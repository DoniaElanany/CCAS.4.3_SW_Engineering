        ...
        if (removed_courses && removed_courses.trim() !== '') {
            ...
            if (coursesToRemove.length > 0) {
                ...
                if (role === 'student') {
                    ...
                    const placeholders = coursesToRemove.map(() => '?').join(',');
                    removeCoursesQuery = `UPDATE courses SET instructor_id = NULL WHERE instructor_id = ? AND course_id IN (${placeholders})`;
                    db.query(removeCoursesQuery, [userId, ...coursesToRemove], (err) => {
                        if (err) {
                            return console.error('Error dissociating instructor from courses:', err);
                        }
                    });
                }
            }
...
app.get('/api/course-details', checkLoggedIn, (req, res) => {
   ...
   `;
    ...
    db.query(query, [courseId], (err, results) => {
        ...
        const materialQuery = `SELECT material_id, title, description, file_path FROM learning_materials WHERE course_id = ?`;
...
app.post('/api/upload-material', checkLoggedIn, upload.fields([{
    ...
    db.beginTransaction(err => {
        ...
        if (materialFile) {
            ...
            const materialQuery = `INSERT INTO learning_materials (course_id, title, description, file_path) VALUES (?, ?, ?, ?)`;
